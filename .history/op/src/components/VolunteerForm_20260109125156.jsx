...
const VolunteerForm = ({ onSubmit, onCancel }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // First create volunteer data
            const volunteerData = {
                _id: `vol_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            ...
            try {
                ...
                if (response.success) {
                    // If backend succeeds, use backend data
                    volunteerData._id = response.data._id;
                    volunteerData.uniqueId = response.data.uniqueId;
                    volunteerData.imageUrl = response.data.imageUrl || formData.imagePreview;
            ...
            if (onSubmit) {
                const success = onSubmit(volunteerData);
                if (success) {
                    resetForm();
                    // Don't navigate here - parent will handle navigation
                }
            }
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-8" encType="multipart/form-data">
                    ...
                    <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
                        <p className="text-sm text-blue-800 flex items-start">
                            ...
                            <span>
                                * After registration, your ID card will be automatically generated. 
                                You can download, print, or share it immediately.
                            </span>
                        </p>
                    </div>
                </form>

...
const AdminDashboard = () => {
  ...
  const [stats, setStats] = useState({
    ...
    president: 0,
    vicePresident: 0,
    soorveerYodha: 0  // Changed from employees
  });
  const [searchTerm, setSearchTerm] = useState('');
  const [roleFilter, setRoleFilter] = useState('all');
  const [selectedVolunteer, setSelectedVolunteer] = useState(null);
  const [showRoleModal, setShowRoleModal] = useState(false);
  const [newRole, setNewRole] = useState('soorveer-yodha');  // Changed default
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch volunteers
      const volunteersRes = await fetch('https://nnnn-x39m.onrender.com/api/admin/volunteers', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleAssignRole = async () => {
    ...
    try {
      const response = await fetch('https://nnnn-x39m.onrender.com/api/admin/assign-role', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        toast.success(`Role updated to ${newRole === 'soorveer-yodha' ? 'Soorveer Yodha' : newRole}`);
  ...
  const handleRemoveRole = async (volunteerId) => {
    ...
    try {
      const response = await fetch('https://nnnn-x39m.onrender.com/api/admin/remove-role', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  // Get role display name - NEW FUNCTION
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-sm">Soorveer Yodha</p>
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option value="soorveer-yodha">Soorveer Yodha</option> {/* Changed */}
                </select>
              </div>
            </div>
          </div>
                  ...
                  >
                      ...
                      <div className="flex items-center space-x-2">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getRoleBadge(volunteer.role)}`}>
                          <span className="flex items-center space-x-1">
                            ...
                            <span>
                              {getRoleDisplayName(volunteer.role)} {/* Updated */}
                            </span>
                          </span>
                        </span>
                        {volunteer.role !== 'soorveer-yodha' && volunteer.assignedBy && ( {/* Changed */}
                    ...
                    <td className="py-3 px-4">
                      <div className="flex space-x-2">
                        ...
                        {volunteer.role !== 'soorveer-yodha' && ( {/* Changed */}
                          <button
              ...
              <div className="mb-6">
                <p className="text-gray-600 mb-4">
                  Current role: <span className="font-semibold">
                    {getRoleDisplayName(selectedVolunteer.role)} {/* Updated */}
                  </span>
                </p>
                
                <div className="space-y-3">
                  {['president', 'vice-president', 'soorveer-yodha'].map((role) => ( {/* Updated */}
                    <label key={role} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
                      ...
                      <div className="flex items-center space-x-2">
                        {getRoleIcon(role)}
                        <span>{getRoleDisplayName(role)}</span> {/* Updated */}
                      </div>
                      {role === 'president' && (
      ...
      <footer className="mt-12 py-6 border-t border-gray-200">
        <div className="max-w-7xl mx-auto px-4 text-center text-gray-500 text-sm">
          ...
          <p className="mt-1">
            ...
            Soorveer Yodha: {stats.soorveerYodha} {/* Added */}
          </p>
        </div>
      </footer>
    </div>
